"""Shared CAPM / IVOL / catalog logic for EASY INVESTMENT (used by app.py / app_classic and app_extended)."""
...
def load_asset_vs_benchmark(asset_ticker, benchmark_ticker, start, end, refresh_run_key=None):
    """refresh_run_key: pass a new value on each Run Analysis to bypass cache (e.g. public Streamlit app)."""
    raw = yf.download(
...
def get_market_snapshot(asset_ticker, refresh_run_key=None):
    """refresh_run_key: new value each analysis run fetches a fresh quote (cache key)."""
    try:
...
def _find_statement_row(df, aliases):
    ...
    for alias in aliases:
        ...
        for k, orig in idx_map.items():
            if al in k or k.startswith(al):
                return df.loc[orig]
    return None


def _latest_two_values(series):
...
@st.cache_data(show_spinner=False)
def fetch_fundamentals_analysis(ticker, refresh_run_key):
    """
    Structured income statement, balance sheet, and cash flow via Yahoo Finance (yfinance).
    refresh_run_key must change on each Run Analysis so results are not reused from Streamlit cache.
    """
    out = {
        "available": False,
        "ticker": ticker,
    ...
    if not ticker or not isinstance(ticker, str):
        out["statement_notes"].append("Invalid ticker for fundamentals.")
        return out
    ...
    try:
        ...
        if income is None or getattr(income, "empty", True):
            income = getattr(tk, "financials", pd.DataFrame())
        balance = getattr(tk, "balance_sheet", None)
        if balance is None or getattr(balance, "empty", True):
            balance = getattr(tk, "balancesheet", pd.DataFrame())
        cashflow = getattr(tk, "cashflow", None)
        if cashflow is None or getattr(cashflow, "empty", True):
            cashflow = pd.DataFrame()
    except Exception as exc:
        out["statement_notes"].append(f"Could not load statements: {str(exc)[:120]}")
        return out

    if (income is None or income.empty) and (balance is None or balance.empty):
        out["statement_notes"].append("No income or balance sheet data returned (common for some tickers or crypto).")
        return out

    out["available"] = True

    rev_row = _find_statement_row(
        income,
        ["total revenue", "operating revenue", "revenue", "net sales", "sales"],
    )
    ni_row = _find_statement_row(
        income,
        [
            "net income",
    ...
    fcf_row = _find_statement_row(cashflow, ["free cash flow"])

    debt_row = _find_statement_row(balance, ["total debt"])
    if debt_row is None:
        ltd = _find_statement_row(balance, ["long term debt"])
        std = _find_statement_row(balance, ["current debt", "short long term debt", "short-term debt"])
        v0_l, v1_l = _latest_two_values(ltd) if ltd is not None else (None, None)
        v0_s, v1_s = _latest_two_values(std) if std is not None else (None, None)
        if v0_l is not None or v0_s is not None:
            d0 = (v0_l or 0.0) + (v0_s or 0.0)
            if v1_l is not None or v1_s is not None:
                d1 = (v1_l or 0.0) + (v1_s or 0.0)
            else:
                d1 = None
            debt_latest = (d0, d1)
        else:
            debt_latest = (None, None)
    else:
        debt_latest = _latest_two_values(debt_row)

    eq_row = _find_statement_row(
        balance,
        [
            "stockholders equity",
    ...
    rev_n, rev_p = _latest_two_values(rev_row)
    ni_n, ni_p = _latest_two_values(ni_row)
    ocf_n, _ = _latest_two_values(ocf_row)
    fcf_n, _ = _latest_two_values(fcf_row)
    debt_n = debt_latest[0]
    eq_n, eq_p = _latest_two_values(eq_row)

    rev_yoy = _yoy_pct(rev_n, rev_p)
    ni_yoy = _yoy_pct(ni_n, ni_p)

    de_ratio = None
    if debt_n is not None and eq_n is not None and abs(eq_n) > 1e-9:
        de_ratio = debt_n / eq_n

    ocf_to_ni = None
    if ocf_n is not None and ni_n is not None and abs(ni_n) > 1e-9:
        ocf_to_ni = ocf_n / ni_n

    m = {
        "revenue_latest": rev_n,
        "revenue_prior": rev_p,
        "revenue_yoy": rev_yoy,
        "net_income_latest": ni_n,
        "net_income_prior": ni_p,
        "net_income_yoy": ni_yoy,
        "debt_to_equity": de_ratio,
        "operating_cash_flow_latest": ocf_n,
        "free_cash_flow_latest": fcf_n,
        "ocf_to_net_income": ocf_to_ni,
        "stockholders_equity_latest": eq_n,
    }
    out["metrics"] = m

    hl = []
    if rev_yoy is not None:
        hl.append(f"Revenue YoY (latest vs prior reported period): **{rev_yoy:+.1%}**")
    if ni_yoy is not None:
        hl.append(f"Net income YoY: **{ni_yoy:+.1%}**")
    if de_ratio is not None:
        hl.append(f"Debt / equity (latest period): **{de_ratio:.2f}**")
    if ocf_to_ni is not None:
        hl.append(f"Operating cash flow / net income: **{ocf_to_ni:.2f}**")
    if fcf_n is not None:
        hl.append(f"Latest reported free cash flow: **{fcf_n:,.0f}** (units as reported)")
    if not hl:
        hl.append("Statements loaded; key lines (revenue, net income) were not matched—see raw data via Yahoo Finance.")
    out["highlights"] = hl

    out["statement_notes"].append(
        "Figures are from Yahoo Finance / yfinance (as-reported style fields; naming varies by issuer)."
    )
    return out


def build_fundamental_signal_components(fundamentals_summary):
    if not fundamentals_summary or not fundamentals_summary.get("available"):
        return []
    m = fundamentals_summary.get("metrics") or {}
    comps = []

    rev_y = m.get("revenue_yoy")
    if rev_y is not None:
        if rev_y >= 0.08:
            comps.append(("Reported revenue grew solidly year-over-year in the latest filing window.", 1.5))
        elif rev_y >= 0.02:
            comps.append(("Reported revenue is up modestly year-over-year.", 0.75))
        elif rev_y <= -0.08:
            comps.append(("Reported revenue declined sharply year-over-year—a fundamental headwind.", -1.75))
        elif rev_y < 0:
            comps.append(("Reported revenue slipped year-over-year.", -0.75))

    ni_y = m.get("net_income_yoy")
    if ni_y is not None:
        if ni_y >= 0.12:
            comps.append(("Net income improved strongly vs the prior period.", 1.5))
        elif ni_y >= 0.03:
            comps.append(("Net income is higher year-over-year.", 0.75))
        elif ni_y <= -0.15:
            comps.append(("Net income fell materially year-over-year.", -1.75))
        elif ni_y < 0:
            comps.append(("Net income is down year-over-year.", -0.75))

    ni = m.get("net_income_latest")
    if ni is not None and ni < 0:
        comps.append(("Latest reported net income is negative.", -1.25))

    de = m.get("debt_to_equity")
    if de is not None:
        if de >= 2.5:
            comps.append(("Balance sheet leverage (debt/equity) looks elevated—refinance and rate risk matter.", -1.25))
        elif de <= 0.35 and de >= 0:
            comps.append(("Debt/equity is relatively light vs many peers (context-dependent).", 0.5))

    ocf_ni = m.get("ocf_to_net_income")
    if ocf_ni is not None and ni is not None and ni > 0:
        if ocf_ni < 0.6:
            comps.append(("Operating cash flow trails net income—earnings quality may need scrutiny.", -1.0))
        elif ocf_ni > 1.1:
            comps.append(("Operating cash flow comfortably covers net income (cash conversion).", 1.0))

    fcf = m.get("free_cash_flow_latest")
    if fcf is not None and ni is not None and ni > 0 and fcf < 0:
        comps.append(("Free cash flow is negative while net income is positive—capex or working capital drag.", -0.75))
...
def build_prediction_signal(
    ...
    *,
    ...
    quant_components = quant_components + build_fundamental_signal_components(fundamentals_summary)
    ...
    if fundamentals_summary and fundamentals_summary.get("available"):
        evidence_count += 2
    ...
    fund_note = ""
    if fundamentals_summary and fundamentals_summary.get("available"):
        fund_note = " reported financial statements (via Yahoo Finance),"
    summary = (
        f"{asset_name} currently leans toward **{action}** with **{confidence.lower()} confidence**. "
        f"The model blends return trend, rolling IVOL behavior,{fund_note} risk profile, and {news_coverage_label}."
    )
