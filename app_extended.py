...
def build_forward_direction_snapshot(metrics, asset_ticker, refresh_run_key=""):
    ...
    if pd.Timestamp(actual_end).date() >= today:
        ...
        fwd_end = max(
            ...
            pd.Timestamp(actual_end) + pd.Timedelta(days=200),
        )
        close = download_asset_close_slice(asset_ticker, actual_end, fwd_end, refresh_run_key)
    ...
    return {"snapshots": snapshots, "error": error}
...
def render_prediction_sense_check(prediction, metrics, asset_ticker, *, show_heading=True, refresh_run_key=""):
    ...
    if show_heading:
        st.markdown("#### Price direction after your sample ended")
    st.caption(
        "**Tomorrow** = next trading session after your end date. **Month / three months** ≈ that many **trading sessions** (not exact calendar months). "
        "Uses past closes only—**not** a promise about the future."
    )
    if out["error"]:
        st.info(out["error"])
        return
    c1, c2, c3 = st.columns(3)
    for col, snap in zip((c1, c2, c3), out["snapshots"]):
        r = snap.get("return")
        delta = f"{r:+.2%}" if r is not None and np.isfinite(r) else None
        col.metric(snap["title"], snap["word"], delta=delta)
...
def render_single_asset_dashboard(
    ...
    with st.expander("Price direction: tomorrow, ~1 month, ~3 months (after sample end)"):
